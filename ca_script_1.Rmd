...
final_dataset %>% 
  filter(item == 'death') %>% 
  mutate(living = 100 * (total_population_living / total_population_tracked),
            dead = 100 * (total_population_dead / total_population_tracked)) %>% 
  select(item, ca_supp, living, dead) %>% 
  gather(cat, pct, c(3,4)) %>% 
  ggplot(aes(pct, color = ca_supp))+
  geom_density()+
...
final_dataset %>% 
  filter(item == 'death') %>% 
  select(item, ca_supp, total_population_living, total_population_dead) %>% 
  gather(population, count, c(3,4)) %>% 
  group_by(ca_supp, population) %>% 
  summarise(average = mean(count)) %>% 
  ggplot(aes(population, average, fill = ca_supp))+
...
crude_rate <- final_dataset %>% 
  ...
  select(item, gender, item_age_group,total_population_dead, total_population_tracked, crude_death_rate_per_100000, ca_supp)

crude_rate %>% 
  group_by(ca_supp) %>% 
  summarise(total_population_dead = sum(total_population_dead))   # Something tells me this is wrong 
